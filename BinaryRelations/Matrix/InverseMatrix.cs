...
namespace MaxRev.Extensions.Matrix
{
    public static partial class MatrixExtensions
    {
        ...
        #region Inverse
        private static bool IsNotZero(this float num) => !num.IsZero();

        private static bool IsZero(this float num)
        {
        ...
        /// <param name="matrix"></param>
        /// <returns></returns>
        public static float Determinant(this float[][] matrix)
        {
        ...
        public static float[] SystemSolve(float[,] A, float[] b)
        {
            ...
            // 2. permute b according to perm[] into bp
            var bp = new float[b.GetLength(0)];
            for (var i = 0; i < n; ++i)
                bp[i] = b[perm[i]];
